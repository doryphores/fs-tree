...
FileTree.prototype.expand = function (nodePath) {
  var node = this.findNode(nodePath);

  if (node.get("type") == "file") {
    throw new Error("Cannot expand File node at path: " + nodePath);
  }

  if (!_.contains(this.expandedPaths, nodePath)) {
    this.reloadNode(nodePath);

    // Start watching any expanded child node
    _.filter(this.expandedPaths, function (p) {
    ...
    this.watchNode(nodePath);
    this.expandedPaths.push(nodePath);
  }
};


FileTree.prototype.collapse = function (nodePath, recursive) {
  if (_.contains(this.expandedPaths, nodePath)) {
    this.unwatchNode(nodePath);
    this.expandedPaths = _.without(this.expandedPaths, nodePath);
    if (recursive) {
      this.expandedPaths = _.reject(this.expandedPaths, function (p) {
        return p.match("^" + nodePath + "(\/|$)");
      })
    }
  }
